...
function buildBasicDependencies(stem) {
    ...
    if (isBadNaming) {
        throw new _gulpUtil.PluginError(PLUGIN_NAME, `Invalid bem naming used: ${ stem }`, { showStack: true });
    }
...
function gulpOrderBemDeps(deps) {
    ...
    const inputPromise = _getStreamAndPromiseF.promise;
    ...
    Promise.all([(0, _collectStreamFiles2.default)(deps), inputPromise]).then(_ref => {
        ...
        let ctx = _ref2$.ctx;
        let closeStreamCallback = _ref2$.closeStreamCallback;
        ...
        const sortOp = new _topologicalSort2.default(mergedNodes);
        ...
        console.error(_gulpUtil.colors.red(err.message));
